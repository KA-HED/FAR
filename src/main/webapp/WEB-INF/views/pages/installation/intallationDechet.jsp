            ...
            <div>
                ...
                <div class="col-12 mt-5"  dir="${pageContext.response.locale=='ar'?'rtl':'ltr'}">
                    ...
                    <div class="dropdown-content1" aria-labelledby="dropdownMenuLink" style="text-align: ${pageContext.response.locale=='ar'?'right':'left'}">
                        <a style="padding: 4px" href="/api/getListeInstallation/0/40/0"><spring:message code='label.Tous'/></a>
                        <c:forEach items="${statut}" var="st">
                            <c:if test="${st.nom_fr!='Brouillon'}">
                                <a style="border-bottom: 1px solid white; padding: 4px"
                                   ...
                                   href="/api/getListeInstallation/0/40/${st.id_statut_projet}">${st[lng] }</a>
                            </c:if>
                        </c:forEach>
                        <a style="border-bottom: 1px solid white; padding: 4px; text-align: ${pageContext.response.locale=='ar'?'right':'left'}"
                           href="/api/getListeInstallation/0/40/1000">Dossier Clôturé</a>
                    </div>
                </div>
                <div class="col-md-12 mt-3 col-lg-12" dir="${pageContext.response.locale=='ar'?'rtl':'ltr'}" style="padding:0">
                    ...
                    <div class="col-md-12 col-lg-12" style="width: 98%;margin-left: auto;margin-right: auto;background: white;padding-top: 19px;border-radius: 5px;display:flex;">
                        <form id="gamesForm" style=" display:flex;" action="/api/getListeInstallationSearch/0/40" method="GET">
...
<%--                                        <option value="/api/getListeInstallation/0/40/0"><spring:message code='label.Tous'/></option>--%>
<%--                                        <c:forEach items="${statut}" var="st">--%>
<%--                                            <c:if test="${st.nom_fr!='Brouillon'}">--%>
<%--                                                <option--%>
<%--                                                    ${statNow==st.id_statut_projet?"selected":""}--%>
<%--                                                        value="/api/getListeInstallation/0/40/${st.id_statut_projet}">${st[lng] }</option>--%>
<%--                                            </c:if>--%>
<%--                                        </c:forEach>--%>
<%--                                    </select>--%>
<%--                                </div></div>--%>
                                ...
                                <div class="col-lg-1 col-sm-12 mb-2">
                                    <a style="padding: 5px;margin-top: 32px;background-color: #2196f3   !important"class="btn btn-primary" onclick="searchByDate1(0)"><spring:message code='label.Rechercher'/></a>
                                </div>
                            </div>
                        </form>
                    </div>
                ...
                <div class="row">
                    <div class="col-12 table-responsive">
                        <table class="table table-hover table-striped table-bordered" id="tab_code">
                            <thead style="background: gray;color: white;border-color: #737373;">
                            <tr>
                                <th style="width: 15% !important"><spring:message code="label.Code"/></th>
...
<script>
    ...
    function searchByDate1(page){
       ...
       /* if(inlineRadioOptions==="" || date1==="" || date2===""){
        ...
        $.ajax({
            url:"/api/getListeInstallationSearch/"+page+"/40",
            type:"GET",
    ...
    function goToDetail_IT(val){
        window.location.href='/api/detail_installation/'+val;
    }
    ...
    function getListFiltrerInstallation(order){
        ...
        $('#loader_modal').modal('show');
        $.ajax({
            url: "/api/getListeInstallationDesc/0/40/"+idStatut+"/"+order,
            type: "GET",
            data: {
            },
            success: function (response) {
    ...
    function EditInstallation(id_installation){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "/api/updateStatutIT/"+id_installation,
                    type: "GET",
                    data: {},
                    success: function (response) {
                        window.location.href="/api/getListeInstallation/0/40/0";
                    },
                    error: function (response) {
                        alert('Erreur ajout non effectué');
                    }
