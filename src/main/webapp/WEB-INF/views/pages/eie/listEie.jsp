...
<div class="container-fluid page-body-wrapper" style="margin: 20px 0px 20px 0px;">
    <div class="main-panel" style="margin-top:150px" >
        <div class="content-wrapper">
            ...
            <c:choose>
                <c:when test="${fn:startsWith(requestScope['javax.servlet.forward.request_uri'], '/api/Renseignement/11/0/40')}">
                </c:when>
                <c:otherwise>
            ...
            <div class="col-12" dir="${pageContext.response.locale=='ar'?'rtl':'ltr'}">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb ml-4" style="margin-top: -18px !important;">
                        ...
                        <li class="breadcrumb-item"><a style="${pageContext.response.locale!='ar'?'color: black;cursor: default':'margin-right:4rem !important;color: black;cursor: default'}" href="/api/getEie/${type}/0/40/0">${titre} </a></li>
                ...
                <div class="dropdown-content1" aria-labelledby="dropdownMenuLink">
                    <a style="padding: 4px" href="/api/getEie/${type}/0/40/0"><spring:message code='label.Tous'/></a>
                    <c:forEach items="${statut}" var="st">
                        <c:if test="${st.nom_fr!='Brouillon'}">
                            <a style="border-bottom: 1px solid white; padding: 4px"
                               ...
                               href="/api/getEie/${type}/0/40/${st.id_statut_projet}">${st[lng] }</a>
                        </c:if>
                    </c:forEach>
                    <a style="border-bottom: 1px solid white; padding: 4px; text-align: ${pageContext.response.locale=='ar'?'right':'left'}"
                       href="/api/getEie/${type}/0/40/1000">Dossier Clôturé</a>
                </div>
            </div>
            <div class="col-12 mt-3">
                <div class="row justify-content-center pt-2 pb-4 rounded m-auto" style="background: white !important;">
                    <div class="col-sm-4">
                        ...
                        <%--                            <option value="/api/getEie/${type}/0/40/0"><spring:message code='label.Tous'/></option>--%>
                        <%--                            <c:forEach items="${statut}" var="st">--%>
                        <%--                                <c:if test="${st.nom_fr!='Brouillon'}">--%>
                        <%--                                    <option--%>
                        <%--                                        ${statNow==st.id_statut_projet?"selected":""}--%>
                        <%--                                            value="/api/getEie/${type}/0/40/${st.id_statut_projet}">${st[lng] }</option>--%>
                        <%--                                </c:if>--%>
                        <%--                            </c:forEach>--%>
                        <%--                            <option--%>
                        <%--                            ${statNow==-1?"selected":""}--%>
                        <%--                                    value="/api/getEie/${type}/0/40/-1">Moin de 3 mois</option>--%>
                        <%--                        </select>--%>
                    </div>
                </div>
            </div>
            <c:if test="${type=='EE'}">
                <div class="col-12 mt-2">
                    <a class="btn float-right" style="background-color: #7dc7bd" href="/api/Renseignement/11/0/40"><spring:message code='label.Renseignementsprealables'/></a>
                </div>
            </c:if>
                ...
                <table style="width:100% !important; " id="tableProfils"
                    ...
                    <tr style="background-color:#2196f3     ">
                        ...
                        <c:if test="${type=='AE'}">
                            ...
                            <th ><spring:message code="label.PVetlistedeprsence"/></th>
                            <th ><spring:message code="label.rapportdelavisite"/></th>
                            <%--<th ><spring:message code="label.consistancedelactivite"/> </th>--%>
                        </c:if>
                    ...
                    <c:forEach items="${all_demande}" var="nt" varStatus="loopp">
                        ...
                        <tr>
                            ...
                            <td>
                                <c:choose>
                                    <c:when test="${nt.statut.id_statut_projet==2 and nt.type.equals('AE')}">
                                       <span class="badge badge-info">Réunion de recevabilité programmée</span>
                                    </c:when>
                                    <c:when test="${nt.statut.id_statut_projet==58 and nt.type.equals('EE')}">
                                        <span class="badge badge-info">En attente des conclusions de l'EP</span>
                                    </c:when>
                                    <c:when test="${nt.statut.id_statut_projet==59 and nt.type.equals('AE')}">
                                        <span class="badge badge-info">En attente du complément du dossier</span>
                                    </c:when>
                                    <c:when test="${nt.statut.id_statut_projet==71 and nt.type.equals('AE')}">
                                        <span class="badge badge-info">Visite de terrain programmée</span>
                                    </c:when>
                                    <c:when test="${nt.statut.id_statut_projet==60 and nt.type.equals('EE')}">
                                        <span class="badge badge-info">En attente du complément de l’EIE</span>
                                    </c:when>
                                    <c:otherwise>
                            ...
                            <td>
                                <c:if test="${pageContext.response.locale=='ar'}">
                                    <span dir="ltr">${fn:replace(nt.tel, " ", "")}</span>
                                </c:if>
                                <c:if test="${pageContext.response.locale!='ar'}">
                                    <span>${fn:replace(nt.tel, " ", "")}</span>
                                </c:if>
                            </td>
                            <td>${nt.fax}</td>
                                ...
                                <td>
                                    <c:choose>
                                        <c:when test="${nt.reunions.size()>0}">
                                            <fmt:parseDate pattern="yyyy-MM-dd" value="${nt.reunions.get(nt.reunions.size()-1).date_reunion}" var="dateVisite" />
                                            <fmt:formatDate dateStyle="long" pattern="dd/MM/yyyy" value="${dateVisite}"/>
                                        </c:when>
                            ...
                            <c:if test="${type=='EE'}">
                                ...
                                <td>
                                    <c:if test="${not empty nt.reunions[nt.reunions.size()-1].url_pv && not empty nt.reunions[nt.reunions.size()-1].url_presence}">
                                        <a class="" style="font-size:21px;color: #33994c;"
                            ...
                            <c:if test="${type=='AE'}">
                                ...
                                <td>
                                    <c:if test="${not empty nt.visites}">
                                        <a class="removeStyle" download
                                           href="${fn:replace(nt.visites[0].visite_url, "/assets/myFile/", "/dowload_uploaded/")}"><span
                                                class="fa fa-download"
                                                style="font-size:21px;color: #33994c;"></span>
                                        </a>
                                    </c:if>
                               ...
                               <%--<td>
                                   <c:if test="${not empty(nt.url_file_AE)}">
                                       <a class="removeStyle" download
                                          href="${fn:replace(nt.url_file_AE, "/assets/myFile/", "/dowload_uploaded/")}"><span
                                               class="fa fa-download"
                                               style="font-size:21px;color: #33994c;"></span>
                            ...
                            <td class="text-center">
                                <div id="toolbar-options${loopp.index+1}" class="hidden">
                                    ...
                                    <c:if test="${nt.statut.id_statut_projet==6 && type!='AE'}">
                                            <a type="button" style="background-color:  #d59d32" onclick="goToDoc('${fn:replace(nt.url_document_signee, "/assets/myFile/", "/dowload_uploaded/")}')" data-popover="true" data-html=true data-content="<spring:message code="label.TelechargeretsignerleautorisationEIE"/>"><i class="fa fa-upload"></i></a>
                                    </c:if>
                                    <c:if test="${nt.statut.id_statut_projet==6 && type=='AE'}">
                                    ...
                                    <c:if test="${nt.type!='NT'}">
                                        ...
                                        <c:if test="${(nt.statut.id_statut_projet==5 && user.poste.id_Poste!=1 && user.poste.id_Poste!=3) || nt.statut.id_statut_projet==5 && user.poste.id_Poste!=1 && user.compteId!=nt.compteAffecte.compteId}">
                                                <a type="button" style="background-color:  #d59d32" onclick="goToLien4('${nt.id_demande_information}')" data-popover="true" data-html=true data-content="<spring:message code="label.TelechargeretsignerleautorisationEIE"/>"><i class="fa fa-upload"></i></a>
                                        </c:if>
...
<script>
    ...
    function listAlimentation(type, page, size) {
        window.location.href = '/api/getEie/' + type + '/' + page + '/40/${statutEnc}';
    }
    ...
    function getListFiltrerEIE(order){
        ...
        $.ajax({
            url: "/api/getEieDesc/${type}/0/40/"+idStatut+"/"+order,
            type: "GET",
            data: {
            },
            success: function (response) {
