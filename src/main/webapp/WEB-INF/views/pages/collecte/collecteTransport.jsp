            ...
            <div>
                ...
                <div class="col-12 "  dir="${pageContext.response.locale=='ar'?'rtl':'ltr'}">
                    ...
                    <div class="dropdown-content1" aria-labelledby="dropdownMenuLink">
                        <a style="padding: 4px" href="/api/getListeCollecte/0/40/0"><spring:message code='label.Tous'/></a>
                        ...
                        <c:forEach items="${statu}" var="st">
                            <c:if test="${st.nom_fr!='Brouillon'}">
                                <a style="border-bottom: 1px solid white; padding: 4px"
                                   ...
                                   href="/api/getListeCollecte/0/40/${st.id_statut_projet}">${st[lng] }</a>

                            </c:if>
                        </c:forEach>
                        <a style="border-bottom: 1px solid white; padding: 4px; text-align: ${pageContext.response.locale=='ar'?'right':'left'}"
                           href="/api/getListeCollecte/0/40/1000">Dossier Clôturé</a>
                    </div>
...
<%--                                         <option value="/api/getListeCollecte/0/40/0"><spring:message code='label.Tous'/></option>--%>
<%--                                         <c:forEach items="${statu}" var="st">--%>
<%--                                             <c:if test="${st.nom_fr!='Brouillon'}">--%>
<%--                                                 <option--%>
<%--                                                     ${statNow==st.id_statut_projet?"selected":""}--%>
<%--                                                         value="/api/getListeCollecte/0/40/${st.id_statut_projet}">${st[lng] }</option>--%>
<%--                                             </c:if>--%>
<%--                                         </c:forEach>--%>
<%--                                     </select>--%>
<%--                                 </div></div>--%>
                                 ...
                                 <div class="col-lg-2 col-sm-12 mt-1" style="margin-top: -2px">
                                 ...
                                 <div class="col-lg-2 col-sm-12 mt-1">
                                     <div class="form-group text-left">
                                         ...
                                         <div class="input-group">
                                             <input required type="text" value="" placeholder="<spring:message code='label.Numerodelademande'/>"
                                 ...
                                 <div class="col-lg-1 col-sm-12 mb-2">
                                     <a style="padding: 5px;margin-top: 32px;background-color: #2196f3   !important;"class="btn btn-primary" onclick="searchByDate(0)"><spring:message code='label.rechercher'/></a>
                                 </div>
                             </div>
                         </form>
                     </div>
                        ...
                        <table style="width:100% !important" id="tbody" class="table table-bordered display responsive nowrap">
                         ...
                         <tr style="background-color: #2196f3   !important;">
                             ...
                             <th class=""><spring:message code="label.Telephone" /></th>
                              <th class=""><spring:message code="label.datereunion" /></th>
                               <th class=""><spring:message code="label.datevisite" /></th>
                         ...
                         <c:forEach items="${collect}" var="n" varStatus="loopp">
                             <c:if test="${n.statut.id_statut_projet!=50}">
                                 <tr>
                                     ...
                                     <c:forEach items="${n.reunions}" var="r" >${r.date_reunion} /
                                     ...
                                     <td>
                                         <div id="toolbar-options${loopp.index+1}" class="hidden">
                                             ...
                                             remaqrue ghazal afffichier pout ous le monde  --%>
                                             <c:if test="${n.statut.id_statut_projet == 28}">
...
<script>
    ...
    function listAlimentation(page, size,statut) {
        window.location.href='/api/getListeCollecte/'+page+'/40'+'/'+statut;
    }
...
console.log(response);
                        ...
                        $(response.vehicules)
                            .each(
                                function(idx, el) {
                                    ...
                                    var tr = "<tr> <td style='direction: rtl'>"
    ...
    function EditCollecte(id_collecte){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "/api/updateStatutCT/"+id_collecte,
                    type: "GET",
                    data: {},
                    success: function (response) {
                        window.location.href="/api/getListeCollecte/0/40/0";
                    },
                    error: function (response) {
    ...
    function searchByDate(page){
       ...
       /* if(inlineRadioOptions==="" || date1==="" || date2===""){
        ...
        $.ajax({
            url:"/api/getListeCollecteSearch/"+page+"/40",
            type:"GET",
    ...
    function goToDetail_CT(val){
        window.location.href='/api/detail_collecte/'+val;
    }
    ...
    function getListFiltrerCollecte(order){
        ...
        $.ajax({
            url: "/api/getListeCollecteDesc/0/40/"+idStatut+"/"+order,
            type: "GET",
            data: {
            },
            success: function (response) {
