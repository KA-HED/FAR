                            ...
                            <div class="col-lg-6 col-md-8 col-sm-12 border mb-5 p-4">
                                ...
                                <div class="form-group mt-4">
                                    <div class="row justify-content-center">
                                        ...
                                        <c:if test="${notification.zf_et!='TR' and notification.statut.id_statut_projet!=40}">
                                            <div class="col-10 text-left">
                                                <label><spring:message code="label.Datedebut"/></label>
                                            <c:if test="${notification.zf_et=='ZF' or notification.zf_et=='ET'}">
                                                <input type="date" class="form-control dt dateDebut" name="date1" id="date1"  placeholder="" required onchange="setDateFin(${notification.classification.id_classification})">
                                            </c:if>
                                            <c:if test="${notification.zf_et!='ZF' and notification.zf_et!='ET'}">
                                                <input type="date" class="form-control dt dateDebut" name="date1" id="date1"  placeholder="" required>
                                            </c:if>
                                            </div>
                                            <div class="col-10 text-left">
                                                <label><spring:message code="label.DateFin"/></label>
                                                <input type="date" class="form-control dt disabled" name="date2" ${notification.zf_et=='ZF'?'readonly':''} id="date2"  min="" placeholder="" required>
                                            </div>
                                        </c:if>
                                        <c:if test="${notification.zf_et=='TR' || notification.statut.id_statut_projet==40}">
                                        <div class="col-10">
...
<script>
   ...
   /* $(document).ready(function () {
    ...
    function setDateFin(classification,type){
        ...
        const result1 = addMonths(dateFin, classification===1?2:12);
        console.log("Date Debut => "+datedebut+" Date Fin => "+result1)
        $('#date2').val(result1);
    }
</script>