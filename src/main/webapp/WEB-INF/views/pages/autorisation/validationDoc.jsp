...
<div class="col-12" dir="${pageContext.response.locale=='ar'?'rtl':'ltr'}">
        <div class="jumbotron p-3">
            Demande numéro : <span class="h4 font-weight-bold" style="color: #3b5998">   ${notif.num_notification}</span><span>
        </span>
        </div>
        ...
        <ol class="breadcrumb ml-4">
            ...
            <c:choose>
                <c:when test="${type=='ZF'}">
                    ...
                    <c:set var="url_mini" value="/api/getListeAutorisation/ZF/0/40/0"/>
                </c:when>

                <c:when test="${type=='ET'}">
                    <spring:message code="label.ImpartationDechetsNondangereuxDunPaysEtrange" var="pp_page"/>
                    <c:set value="/api/getListeAutorisation/ET/0/40/0" var="url_mini"/>
                </c:when>
                <c:when test="${type=='XD'}">
                    <spring:message code="label.ExportationDesDechetsDangereux" var="pp_page"/>
                    <c:set value="/api/getListeAutorisation/XD/0/40/0" var="url_mini"/>

                </c:when>
                <c:when test="${type=='TR'}">
                    <spring:message code="label.TransitDesDechets" var="pp_page"/>
                    <c:set value="/api/getListeAutorisation/TR/0/40/0" var="url_mini"/>
                </c:when>
                        ...
                        <div class="table-responsive col-12">
                            <table class="table table-striped table-hover table-bordered ">
                                ...
                                <c:forEach items="${doc}" var="dc">
                                    ...
                                    <tr>
                                        <td> <a target="_blank" download href="${fn:replace(dc.url, "/assets/myFile/", "/api/dowload_uploaded/")}" class="btn btn-success f-18 rounded-circle btn_icon"> <span class="fa fa-upload"></span></a> </td>
    ...
    if (test) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                window.location = "/api/recap_autorisation/" + type + "/"+id;
            },
            error: function (response) {
                alert(' ajout non effectué');
            }
