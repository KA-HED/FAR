            ...
            <div id="table_resultat">
                ...
                <div class="col-12" dir="${pageContext.response.locale=='ar'?'rtl':'ltr'}">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb ml-4" style="margin-top: 17px !important;">
                            ...
                            <li class="breadcrumb-item active" style="${pageContext.response.locale!='ar'?'':'margin-right:4rem !important'}" aria-current="page">${p_page} ${classification=="1"?'Dangeureux':'Non dangreux' }</li>
                ...
                <div class="dropdown-content1 bg-white" aria-labelledby="dropdownMenuLink" style="display: none;">
                    <c:if test="${type.equals('ZF')}">
                        <a style="padding: 4px; text-align: ${pageContext.response.locale=='ar'?'right':'left'}" href="/api/getListeAutorisationZF/${type}/0/40/0/${classification}"><spring:message code='label.Tous'/></a>
                        <c:forEach items="${statut}" var="st">
                            <c:if test="${st.nom_fr!='Brouillon' && st.nom_fr!='Brouillon1'}">
                                <a style="border-bottom: 1px solid white; padding: 4px; text-align: ${pageContext.response.locale=='ar'?'right':'left'}" ${statNow==st.id_statut_projet?"selected":"" }
                                   href="/api/getListeAutorisationZF/${type}/0/40/${st.id_statut_projet}/${classification}">${st[lng] }</a>
                            </c:if>
                        </c:forEach>
                        <a style="border-bottom: 1px solid white; padding: 4px; text-align: ${pageContext.response.locale=='ar'?'right':'left'}"
                           href="/api/getListeAutorisationZF/${type}/0/40/1000/${classification}">Dossier Clôturé</a>
                    </c:if>
                    <c:if test="${!type.equals('ZF')}">
                        <a style="padding: 4px; text-align: ${pageContext.response.locale=='ar'?'right':'left'}" href="/api/getListeAutorisation/${type}/0/40/0"><spring:message code='label.Tous'/></a>
                        <c:forEach items="${statut}" var="st">
                            <c:if test="${st.nom_fr!='Brouillon' && st.nom_fr!='Brouillon1'}">
                                <a style="border-bottom: 1px solid white; padding: 4px; text-align: ${pageContext.response.locale=='ar'?'right':'left'}" ${statNow==st.id_statut_projet?"selected":"" }
                                   href="/api/getListeAutorisation/${type}/0/40/${st.id_statut_projet}">${st[lng] }</a>
                            </c:if>
                        </c:forEach>
                        <a style="border-bottom: 1px solid white; padding: 4px; text-align: ${pageContext.response.locale=='ar'?'right':'left'}"
                           href="/api/getListeAutorisation/${type}/0/40/1000">Dossier Clôturé</a>
                    </c:if>
                </div>
...
<%--                                <option value="/api/getListeAutorisation/${type}/0/40/0"><spring:message code='label.Tous'/></option>--%>
<%--                                <c:forEach items="${statut}" var="st">--%>
<%--                                    <c:if test="${st.nom_fr!='Brouillon'}">--%>
<%--                                        <option--%>
<%--                                            ${statNow==st.id_statut_projet?"selected":""}--%>
<%--                                                value="/api/getListeAutorisation/${type}/0/40/${st.id_statut_projet}">${st[lng] }</option>--%>
<%--                                    </c:if>--%>
<%--                                </c:forEach>--%>
<%--                            </select>--%>
<%--                                </div></div>--%>
                                ...
                                <div class="col-lg-2 col-sm-12 mt-1">
                                    <div class="form-group text-left">
                                        ...
                                        <div class="input-group">
                                            <input required type="text" value="" placeholder="<spring:message code='label.Numerodenotification'/>"
                                                   class="form-control" id="NumNotif" name="NumNotif">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-2 col-sm-12 mt-1">
                                    <div class="form-group text-left">
                                        ...
                                        <div class="input-group">
                                            <input required type="text" value="" placeholder="<spring:message code='label.nomsociete'/>"
                                                   class="form-control" id="NomSociete" name="NomSociete">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-1 col-sm-12 mt-1 mb-2">
                                    <a style="margin-top: 32px;background-color: #2196f3   !important;" class="btn btn-primary"
                                ...
                                <c:forEach items="${notif}" var="nt" varStatus="loopp">
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <c:if test="${nt.statut.id_statut_projet != 37 && nt.dateAvisDepartement != null}">
                                                <fmt:parseNumber integerOnly="true"
                                                        value="${(now.time - nt.dateAvisDepartement.time) / (1000*60*60*24) }" var="tt"
                                                />
                                        ...
                                        <td class=""><c:if test="${nt.statut.id_statut_projet == 39 && nt.zf_et.equals('ET')}">
                                            ...
                                            <c:if test="${nt.statut.id_statut_projet == 105}">
                                        ...
                                        <td class="text-center">
                                            <div id="toolbar-options${loopp.index+1}" class="hidden">
                                                ...
                                                <c:if test="${(nt.statut.id_statut_projet==38 && type!='XD' && type!='TR'  && user.poste.id_Poste!=3) || nt.statut.id_statut_projet==38 && type!='XD' && type!='TR'}">
                                                ...
                                                <c:if test="${(nt.statut.id_statut_projet==40 && type=='ZF' && user.poste.id_Poste!=1 && user.poste.id_Poste!=3) || nt.statut.id_statut_projet==40 && type=='ZF'}">
                                                ...
                                                <c:if test="${(nt.statut.id_statut_projet==40 && type=='ET' && user.poste.id_Poste!=1 && user.poste.id_Poste!=3) || nt.statut.id_statut_projet==40 && type=='ET' }">
                                                    <a type="button" style="background-color:  #d59d32" onclick="goToLien1('${type}/${nt.id_notification}')" data-popover="true" data-html=true data-content="<spring:message code="label.Telechargerlautorisationderefus"/>"><i class="fa fa-signature"></i></a>
                                                </c:if>
                                                <c:if test="${(nt.statut.id_statut_projet==85 && user.poste.id_Poste!=1 && user.poste.id_Poste!=3) || (nt.statut.id_statut_projet==85 )}">
                                                ...
                                                <a type="button" style="background-color:  #d59d32" onclick="goToLiens('${nt.id_notification}')" data-popover="true" data-html=true data-content="Conculter les documents"><i class="fa fa-eye"></i></a>
<%-- <c:if test="${user.compteId==1 }">
 <a type="button" style="background-color:  #d59d32" onclick="deleteAuto('${nt.id_notification}')" data-popover="true" data-html=true data-content="Supprimer la demande"><i class="fa fa-trash"></i></a>
...
document.getElementById('id_select').onchange = function(){
    ...
    function listAlimentation(page, size, type, statut) {
        ...
        if(lien.includes("/api/getListeAutorisationZF/")){
            window.location.href = '/api/getListeAutorisationZF/' + type + '/' + page + '/40/' + statut+"/"+${classification};
        }
        else if(lien.includes("/api/getListeAutorisation/")){
            window.location.href = '/api/getListeAutorisation/' + type + '/' + page + '/40/' + statut;
        }
        else{
            window.location.href = '/api/getListeAutorisationSearch/' + type + '/' + page + '/40/'+statut;
        }
    }
    ...
    function getStat10() {
        ...
        $.ajax({
            url: "/api/getListeAutorisationSearch/ZF/0/40/"+${statutEnc},
            type: "GET",
            ...
            success: function (response) {
                getFunctionParam('getParamDoc/0/40');
    ...
    function searchByDate1(page) {
       ...
       /* if(inlineRadioOptions==="" || date1==="" || date2===""){
        ...
        $.ajax({
            url: "/api/getListeAutorisationSearch/${type}/"+page+"/40/"+${statutEnc},
            type: "GET",
    ...
    function goToLiens(val){
        window.location.href='/api/validateDocAll/'+val;
    }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    data: {},
                    success: function (response) {
                        window.location.href="/api/getListeAutorisation/ZF/0/40/0";
                    },
                    error: function (response) {
                        alert('Erreur ajout non effectué');
                    }
    ...
    function getListFiltrer(order){
       ...
       // var idStatut = url.substring(url.lastIndexOf("/") + 1, url.length);
        var idStatut = '${statutEnc}';
        $('#loader_modal').modal('show');
        var type = "${type}";
        var lien = type!=="ZF"?"/api/getListeAutorisationDesc/${type}/0/40/"+idStatut+"/"+order:"/api/getListeAutorisationDescZF/${type}/0/40/"+idStatut+"/"+order+"/"+${classification}
        $.ajax({
            url: lien,
            type: "GET",
            data: {
